...
func (j *Job) GetBuild(ctx context.Context, id int64) (*Build, error) {
	...
	// j.Raw.URL will be "https://<domain>/jenkins/job/JOB1/", so we need to remove the last "/"
	// and then add / to the prefix
	jobURL := "/" + strings.Trim(strings.Replace(j.Raw.URL, j.Jenkins.JenkinsURL, "", -1), "/")
